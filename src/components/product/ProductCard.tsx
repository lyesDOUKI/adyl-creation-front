  ...
  return (
    <Card className="group overflow-hidden shadow-card hover-lift border-primary/5 hover:border-primary/20 bg-card h-full flex flex-col">
      ...
      >
        ...
        <button
          className={cn(
            ...
            liked ? 'bg-primary text-primary-foreground shadow-glow scale-110' : 'bg-card/90 text-muted-foreground hover:bg-primary/10 hover:text-primary'
          )}
          onClick={(e) => { e.stopPropagation(); setLiked(!liked); }}
        >
          <Heart className={cn('h-4 w-4 transition-all', liked && 'fill-current animate-heartbeat')} />
        </button>

        {product.isCustomizable && (
          <Badge className="absolute top-3 left-3 bg-primary/90 text-primary-foreground hover:bg-primary text-xs font-body shadow-warm">
            ✨ Personnalisable
          </Badge>
        )}
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col">
        {/* Rating */}
        <div className="flex items-center gap-1 text-sm mb-2.5">
          <Star className="h-3.5 w-3.5 fill-primary/80 text-primary/80" />
          <span className="font-semibold text-sm">{product.rating}</span>
          <span className="text-muted-foreground text-xs">({product.reviewCount} avis)</span>
        </div>
        ...
        {/* Price */}
        <p className="text-lg font-bold text-primary mb-2.5">{formatPrice(product.price)}</p>
